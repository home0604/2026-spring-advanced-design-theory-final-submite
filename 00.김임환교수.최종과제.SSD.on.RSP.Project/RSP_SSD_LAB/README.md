...
Raspberry Pi에서 가위바위보 손 모양을 SSD Object Detection 방식으로 실행하고, TFLite 양자화 전후의 모델 크기, 추론 시간, CPU/RAM 사용량을 비교하는 실습 repo다.
...
이 모델은 TFLite 형식이라 온디바이스 실행에 적합하지만, int8 양자화 최경량 모델은 아니다.

양자화 비교에는 별도의 작은 SSD-Lite 모델을 사용한다.
...
블로그에서 `RSP_SSD_LAB.zip`을 받아 압축을 푼다.
...
이미 수업용 이미지에 `~/camera_test/.venv311` 환경이 준비되어 있으면 새로 만들 필요 없다. 아래로 확인한다.
...
실행되면 카메라 영상 위에 `SCISSORS`, `ROCK`, `PAPER` 바운딩 박스가 표시된다.
...
RSS는 Resident Set Size의 약자이며, 현재 프로세스가 실제 RAM에 올려 사용 중인 메모리 크기다.
